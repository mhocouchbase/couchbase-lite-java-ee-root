...
echo "======== TEST Couchbase Lite Android, Enterprise Edition v`cat ../../version.txt`-${BUILD_NUMBER} on device: ${ANDROID_SERIAL}"
...
pushd test/build/reports/androidTests/connected/flavors
zip -r "${REPORTS}/test-reports-android-ee" debugAndroidTest
find "${REPORTS}"
popd
