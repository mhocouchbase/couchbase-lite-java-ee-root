...
# undef sqlite3_key
# undef sqlite3_rekey
# define sqlite3CodecAttach  sqlite3CodecAttach_aes256ofb
# define sqlite3CodecGetKey  sqlite3CodecGetKey_aes256ofb
# define sqlite3_key         sqlite3_key_aes256ofb
# define sqlite3_rekey       sqlite3_rekey_aes256ofb
#endif
...
*/
typedef struct CodecAes256 CodecAes256;
struct CodecAes256 {
  struct KeyAes256 {
  ...
  u8 mallocFailed;               /* True if a memory allocation has been seen */  int nonceZero;                 /* Add this many zeros to end of nonce */
...
*/
static int rijndaelKeySetupEnc256(u32 *rk, const u8 *cipherKey){
  int i = 0;
  u32 temp;
...
*/
const char sqlite3_Copyright_AES256[] = 
...
*/
static void *sqliteCodecAes256(
  void *codecState,
  void *pageData,
  Pgno pgno,
  int op
){
  CodecAes256 *pCodec = codecState;
  ...
  */  
  if( op&2 ){
    /* AES with a 256-bit key and 128-bit block in OFB mode */
    unsigned int i;
...
*/
static void loadKey256(CodecAes256 *pCodec, const void *pKey, int nKey){
  if( pKey && nKey!=0 ){
    ...
    rijndaelKeySetupEnc256(pCodec->key[0].keySchedule, pCodec->key[0].repeat);
  }else{
...
*/
static void sqliteCodecAes256SizeChng(void *p, int pageSize, int nReserve){
  CodecAes256 *pCodec = (CodecAes256*)p;
...
** Deallocate a codec.
*/
static void sqliteCodecAes256Free(void *p){
  CodecAes256 *pCodec = (CodecAes256*)p;
  sqlite3_free(pCodec->mask);
  memset(p, 0, sizeof(CodecAes256));
  sqlite3_free(p);
}
...
int sqlite3CodecAttach(
  ...
  if( pDb->pBt && (pPager = sqlite3BtreePager(pDb->pBt))!=0 ){
    pCodec = sqlite3_malloc( sizeof(CodecAes256) );
    if( pCodec==0 ){
      return SQLITE_NOMEM;
    }
    memset(pCodec, 0, sizeof(CodecAes256));
    /* 12 bytes of nonce for AES */
    sqlite3BtreeSetPageSize(pDb->pBt, 0, AES_BLOCK_SZ-sizeof(Pgno), 0);
    loadKey256(pCodec, pKey, nKey);
    pCodec->key[1] = pCodec->key[0];
    sqlite3PagerSetCodec(pPager,
         sqliteCodecAes256,
         sqliteCodecAes256SizeChng,
         sqliteCodecAes256Free,
         pCodec
    );
  }
  return SQLITE_OK;
...
void sqlite3CodecGetKey(sqlite3 *db, int iDb, void **ppKey, int *pnKey){
  ...
  pCodec = (CodecAes256*)sqlite3PagerGetCodec(pPager);
  if( pCodec==0 ){
    *pnKey = 0;
    *ppKey = 0;
  }else{
