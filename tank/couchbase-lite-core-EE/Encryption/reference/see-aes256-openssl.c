...
** the AES encryption algorithms from the OpenSSL library.
**
...
# undef sqlite3_key
# undef sqlite3_rekey
# define sqlite3CodecAttach  sqlite3CodecAttach_openssl
# define sqlite3CodecGetKey  sqlite3CodecGetKey_openssl
# define sqlite3_key         sqlite3_key_openssl
# define sqlite3_rekey       sqlite3_rekey_openssl
#endif
#define KEY_SZ          32           /* Size of the AES-256 key in bytes */
#define AES_BLOCK_SZ    16           /* Size of each encryption block */

/*
** State information used by the codec
*/
typedef struct CodecOpenssl CodecOpenssl;
struct CodecOpenssl {
  struct KeyOpenssl {
  ...
  EVP_CIPHER_CTX *ctx;           /* OpenSSL cypher context */
};

/*
** Make sure "strings" turns up a copyright notice
*/
const char sqlite3_Copyright_Openssl[] = 
  "Copyright 2004-2019 Hipp, Wyrick & Company, Inc. "
...
*/
static void *sqliteCodecOpenssl(
  void *codecState,
  void *pageData,
  Pgno pgno,
  int op
){
  CodecOpenssl *pCodec = codecState;
  ...
  */
  if( op&2 ){
    int nOut = 0;
    EVP_EncryptInit_ex(pCodec->ctx, NULL, NULL, 
                       pCodec->key[op&1].repeat, nonce);
    EVP_EncryptUpdate(pCodec->ctx, pCodec->mask, &nOut,
                      pCodec->pZero, pCodec->pageSize);
    OPENSSL_assert( nOut==pCodec->pageSize );
  }
...
*/
static void loadKeyOpenssl(CodecOpenssl *pCodec, const void *pKey, int nKey){
  if( pKey && nKey!=0 ){
...
*/
static void sqliteCodecOpensslSizeChng(void *p, int pageSize, int nReserve){
  CodecOpenssl *pCodec = (CodecOpenssl*)p;
...
** Deallocate a codec.
*/
static void sqliteCodecOpensslFree(void *p){
  CodecOpenssl *pCodec = (CodecOpenssl*)p;
  sqlite3_free(pCodec->mask);
  EVP_CIPHER_CTX_free(pCodec->ctx);
...
int sqlite3CodecAttach(
  ...
  if( pDb->pBt && (pPager = sqlite3BtreePager(pDb->pBt))!=0 ){
    pCodec = sqlite3_malloc( sizeof(CodecOpenssl) );
    if( pCodec==0 ){
      return SQLITE_NOMEM;
    }
    memset(pCodec, 0, sizeof(CodecOpenssl));
    pCodec->ctx = EVP_CIPHER_CTX_new();
    EVP_EncryptInit_ex(pCodec->ctx, EVP_aes_256_ofb(), NULL, NULL, NULL);
    sqlite3BtreeSetPageSize(pDb->pBt, 0, AES_BLOCK_SZ-sizeof(Pgno), 0);
    loadKeyOpenssl(pCodec, pKey, nKey);
    pCodec->key[1] = pCodec->key[0];
    sqlite3PagerSetCodec(pPager,
         sqliteCodecOpenssl,
         sqliteCodecOpensslSizeChng,
         sqliteCodecOpensslFree,
         pCodec
    );
  }
  return SQLITE_OK;
...
void sqlite3CodecGetKey(sqlite3 *db, int iDb, void **ppKey, int *pnKey){
  ...
  pCodec = (CodecOpenssl*)sqlite3PagerGetCodec(pPager);
  if( pCodec==0 ){
    *pnKey = 0;
    *ppKey = 0;
  }else{
