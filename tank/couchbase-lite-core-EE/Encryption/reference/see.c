...
** any of the following encryption modes:
**
**    * RC4
**    * AES128 in OFB mode.
**    * AES256 in OFB mode.
**
** The encryption mode is selected by a prefix string on the key and
** defaults to AES128-OFB.
**
...
#define KEY_MX 256           /* Maximum passphrase size */
#define KEY_SZ 256           /* Maximum length of key */
#define KEY_SCHED_SZ 68      /* Space to hold the key schedule */
...
*/
static const int nNonce[] = { 4, 12, 12, 28 };
...
*/
typedef struct CodecAny CodecAny;
struct CodecAny {
  struct KeyAny {
    u16 nByte;                   /* Number of bytes in key + prefix */
    u8 nullKey;                  /* Do not encrypt if true */
    u8 alg;                      /* Encryption algorithm to use */
    u8 nPrefix;                  /* Length of key prefix */
    u32 keySchedule[KEY_SCHED_SZ];    /* Rijndael key schedule */
    unsigned char repeat[KEY_MX+8];   /* Key text + extra space for prefix */
  } key[2];                      /* 2 keys.  0: new, 1: old */
  u8 nullKey;                    /* True if mask[] is zero */
  u8 nonceSize;                  /* The amount of nonce per page */
  u8 nonceZero;                  /* Extra zeros added to end of nonce */
...
static const u32 rcon[] = {
    ...
    /* for 256-bit blocks, Rijndael never uses more than 7 rcon values */
};
...
*/
static void rijndaelKeySetupEnc128(u32 *rk, const u8 *cipherKey){
  int i = 0;
  u32 temp;
...
*/
const char sqlite3_Copyright_ANY[] = 
...
*/
static void *sqliteCodecAny(
  void *codecState,
  void *pageData,
  Pgno pgno,
  int op
){
  CodecAny *pCodec = codecState;
  ...
  if( op&2 ){
    switch( alg ){
      ...
      case SEE_AES128OFB: {
        ...
        if( pCodec->nonceSize<12 ){
          memset(&nonce[4+pCodec->nonceSize], 0, 12-pCodec->nonceSize);
        }
      ...
      case SEE_AES256OFB: {
        /* AES with a 256-bit key and block in OFB mode */
        unsigned int i;
        unsigned char *mask = pCodec->mask;
        u32 *rk = pCodec->key[op&1].keySchedule;
        ...
        rijndaelEncrypt256(rk, nonce, mask);
        for(i=16; i<pCodec->usable; i+=16){
          rijndaelEncrypt256(rk, &mask[i-16], &mask[i]);
        }
        break;
      }
    }
...
*/
static void pwHash(const unsigned char *zPassPhrase, int N, unsigned char *aBuf){
  unsigned char i, j, t;
  int m, n;
  unsigned char s[256];
...
*/
static void loadKeyAny(CodecAny *pCodec, const void *pKey, int nKey){
  int usePwHash = 0;
  if( nKey<0 ){
    size_t n = strlen(pKey);
    nKey = n<=KEY_MX ? (int)n : KEY_MX;
    usePwHash = 1;
  }
  if( pKey && nKey>0 ){
    ...
    int i;
    u8 nPrefix = 0;
    u8 alg = SEE_AES128OFB;
    int sz;
    const char *zPrefix = (const char*)pKey;
    ...
    nKey = 4;
#endif
    pCodec->key[0].nPrefix = nPrefix;
    memcpy(pCodec->key[0].repeat, zPrefix, nPrefix);
    sz = szKey[alg];
    if( usePwHash ){
      pwHash(zKey, sz, &pCodec->key[0].repeat[nPrefix]);
      nKey = sz;
    }else{
      if( nKey>sz ) nKey = sz;
      for(i=0; i<sz; i++){
        pCodec->key[0].repeat[i+nPrefix] = zKey[i%nKey];
      }
    }
    pCodec->key[0].nByte = nKey + nPrefix;
    pCodec->key[0].nullKey = 0;
    pCodec->key[0].alg = alg;
    if( alg==SEE_AES128OFB ){
      rijndaelKeySetupEnc128(pCodec->key[0].keySchedule, 
                             &pCodec->key[0].repeat[nPrefix]);
    }else if( alg==SEE_AES256OFB ){
      rijndaelKeySetupEnc256(pCodec->key[0].keySchedule, 
                             &pCodec->key[0].repeat[nPrefix]);
    }
  }else{
...
*/
static void sqliteCodecAnySizeChng(void *p, int pageSize, int nReserve){
  CodecAny *pCodec = (CodecAny*)p;
  pCodec->pageSize = pageSize;
  pCodec->nonceSize = (u8)nReserve;
...
** Deallocate a codec.
*/
static void sqliteCodecAnyFree(void *p){
  CodecAny *pCodec = (CodecAny*)p;
...
**
** This routine attempts to configure the database so that it has extra
...
int sqlite3CodecAttach(
  ...
  if( pDb->pBt && (pPager = sqlite3BtreePager(pDb->pBt))!=0 ){
    pCodec  = sqlite3_malloc( sizeof(CodecAny) );
    if( pCodec==0 ){
      return SQLITE_NOMEM;
    }
    memset(pCodec, 0, sizeof(*pCodec));
    loadKeyAny(pCodec, pKey, nKey);
    sqlite3BtreeSetPageSize(pDb->pBt, 0, nNonce[pCodec->key[0].alg], 0);
    pCodec->key[1] = pCodec->key[0];
    sqlite3PagerSetCodec(pPager,
         sqliteCodecAny,
         sqliteCodecAnySizeChng,
         sqliteCodecAnyFree,
         pCodec
    );
  }
...
void sqlite3CodecGetKey(sqlite3 *db, int iDb, void **ppKey, int *pnKey){
  ...
  pCodec = (CodecAny*)sqlite3PagerGetCodec(pPager);
  if( pCodec==0 ){
    *pnKey = 0;
    *ppKey = 0;
  }else{
...
/*
** For Tizen, the default behavior is as if being compiled as a DLL
*/
#if defined(SQLITE_TIZEN) || defined(SEE_EXPIRATION)
# define SQLITE_DLL 1
#endif
...
#else /* if defined(SQLITE_TIZEN) */

SQLITE_API void sqlite3_activate_see(const char *zPassPhrase){
  /* no-op */
}

/* This routine checks to see if SEE is running on a (licensed) Tizen
** system, and enables encryption if it is. */
#include <sys/types.h>
#include <sys/stat.h>
#include <fcntl.h>
#include <unistd.h>
static int sqlite3CheckForTizen(void){
  char zBuf[300];
  int fd = open("/proc/version", O_RDONLY);
  int got, i;
  if( fd<0 ) return 0;
  got = (int)read(fd, zBuf, sizeof(zBuf)-1);
  close(fd);
  if( got<=10 ) return 0;
  zBuf[got] = 0;
  if( memcmp(zBuf, "Linux vers", 10)!=0 ) return 0;
  for(i=10; i<got; i++){
    if( zBuf[i]=='T' && memcmp(zBuf+i, "Tizen ", 6)==0 ){
      encryptionEnabled = 1;
      return 1;
    }
  }
...
#endif
  if( !encryptionEnabled && !sqlite3CheckForTizen() ){
